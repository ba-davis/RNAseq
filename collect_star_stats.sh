...
for i in $dir/*.star.Log.final.out; do
  ...
  grep -l "Number of input reads" $filename | sed -e 's/.star.Log.final.out//g' | sed -e "s|$dir||g" | sed -e 's/\///g' >> tmp_sample_name
...
paste -d "\t" tmp_sample_name tmp_input_reads tmp_unq_number tmp_unq_percent tmp_multi_number tmp_multi_percent tmp_too_many_loci_number tmp_too_many_loci_percent tmp_too_many_mismatch_percent tmp_too_short_percent tmp_unmapped_other_percent > summary_star.txt

sed -i '1 i\SampleName\tInputReads\tUnqMap\tUnqMapPerc\tMultiMap\tMultiMapPerc\tTooManyLoci\tTooManyLociPerc\tUnmappedTooManyMismatch\tUnmappedTooShort\tUnmappedOther' summary_star.txt
